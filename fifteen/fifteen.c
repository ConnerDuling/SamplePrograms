...
{
    int total = d * d - 1;
    //Flag to track if tile count is odd.
    int tileOddFlag = 1;
    if (total % 2 == 0)
        //If the total is even, set flag to false.
        tileOddFlag = 0;
    ...
    //board[d-1][d-2] is the default position for 1
    //board[d-1][d-3] is the default position for 2
    //d-num adjusts for 0 indexing, and for postion.
    if (tileOddFlag)
    {
...
short won(void)
{
    ...
    for (int row = 0; row < d; row++)
    {
        for (int col = 0; col < d; col++)
        {
            if (board[row][col] == counter + 1 || (row == d - 1 && col == d - 1))
            {
                counter++;
            }
            else
